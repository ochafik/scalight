...
trait PartialFunction[-A, +B] extends (A => B) { self =>
   ...
   */
  override def andThen[C](k: B => C) = new scalaLight.PartialFunction[A, C] {
   ...
   */
  //def lift: A => scalaLight.Option[B] = new Lifted(this)
...
object PartialFunction {
   ...
   */
  private final class OrElse[-A, +B] (f1: scalaLight.PartialFunction[A, B], f2: scalaLight.PartialFunction[A, B]) extends scalaLight.PartialFunction[A, B] {
    ...
    override def orElse[A1 <: A, B1 >: B](that: scalaLight.PartialFunction[A1, B1]) =
  ...
  private[scalaLight] lazy val FallbackToken: scalaLight.PartialFunction[Any, Nothing] = 
  new scalaLight.PartialFunction[Any, Nothing] {
    override def isDefinedAt(v: Any) = true
    override def apply(v: Any) =
      FallbackToken.asInstanceOf[Nothing] 
  }
  private[scalaLight] final def fallbackToken[B] = FallbackToken.asInstanceOf[PartialFunction[Any, B]]
  //TODO: check generated code for PF literal here

  /*
  private[scalaLight] final class Lifted[-A, +B] (val pf: scalaLight.PartialFunction[A, B])
      extends (A => B) with scalaLight.runtime.AbstractFunction1[A, scalaLight.Option[B]] {

    def apply(x: A): scalaLight.Option[B] = pf.applyOrElse(x, fallbackToken) match {
      case FallbackToken => scalaLight.None
      case z => scalaLight.Some(z)
    }
  }

  private final class Unlifted[A, B] (f: A => scalaLight.Option[B]) extends runtime.AbstractPartialFunction[A, B] {
  ...
  private[scalaLight] def unlifted[A, B](f: A => scalaLight.Option[B]): scalaLight.PartialFunction[A, B] = f match {
...
*/
   ...
   */
  //TODO: check generated code for PF literal here
  def apply[A, B](f: A => B): scalaLight.PartialFunction[A, B] = new scalaLight.PartialFunction[A, B] {
    override def isDefinedAt(v: A) = true
  ...
  private[this] final val empty_pf: scalaLight.PartialFunction[Any, Nothing] = new scalaLight.PartialFunction[Any, Nothing] {
    def isDefinedAt(x: Any) = false
    def apply(x: Any) = throw new MatchError(x)
    override def orElse[A1, B1](that: scalaLight.PartialFunction[A1, B1]) = that
   ...
   *  @since   2.10
   */
  def empty[A, B] : scalaLight.PartialFunction[A, B] = empty_pf
   ...
   */
  def cond[T](x: T)(pf: scalaLight.PartialFunction[T, Boolean]): Boolean = pf.applyOrElse(x, constFalse)
   ...
   */
  //def condOpt[T,U](x: T)(pf: scalaLight.PartialFunction[T, U]): scalaLight.Option[U] = pf.lift(x)
}
